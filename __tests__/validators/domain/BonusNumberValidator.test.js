...
describe('BonusNumberValidator 클래스 테스트', () => {
  describe('구매금액 유효성 검사 실패 테스트', () => {
    ...
    test.each([
      [mockLottoList, testLottoNumber, ERROR_MESSAGES.LOTTO_RANGE],
      [mockLottoList, mockLottoList[1], ERROR_MESSAGES.LOTTO_DUPLICATE],
    ])('❌ validate 테스트 %s %sthrow Error %s', (numbers, number, errorMessage) => {
